...
super_plot2 = function (ingredients, labels, type = "main", i, j, z, x_axis, 
                        y_axis, subset, simple = "True", k = 0.7, nudge_x = NULL, 
                        nudge_y = NULL, repel = FALSE, push = 1, pull = 1, seed = 622) 
{
    if (type == "main") {
        gg <- main_plot2(ingredients, i, j, z, x_axis, y_axis, subset, k, simple)
    }
    else if (type == "cor") {
        gg <- cor_plot2(ingredients, i, j, z, x_axis, y_axis, subset, k, simple)
        annotate <- add_annotation(labels, i, j, z, x_axis, y_axis, 
            subset, simple, k, nudge_x, nudge_y, repel, push = 1, 
            pull = 1, seed = seed)
        gg + annotate
    }
}

main_plot2 = function (gold, i, j, z, x_axis, y_axis, subset = NULL, k = 0.7, simple = "True") 
{
    gold <- exhaustive_plots_filter(gold, i, j,z, x_axis, subset, k)
    groups_to_labels = data.frame(names = c("Clinical", "HLA", "RNA", "Somatic", "CNV/SVs"),
                                  labels = c("Clinical", "HLA", "Expression", "SNVs/Indels", "CNV/SVs"))
    aesthetic <- aes(x = .data[[x_axis]], y = .data[[y_axis]], 
                     fill = big_group, color = pval_significance,
                     alpha = pval_significance, size = pval_significance)
    
    gg <- (ggplot(gold, aesthetic) + geom_point(shape = 21, color = "white") + 
        scale_color_manual(values = unlist(color_map_significance)) + 
        scale_fill_manual(values = unlist(color_map_type),
                          name = "Group") +
        scale_alpha_manual(values = unlist(alpha_map_significance)) + 
        scale_size_manual(values = unlist(size_map_significance)) + 
        labs(x = x_labeller(i, "plot_est"), y = "-Log10 p-value"))
    gg
}

cor_plot2 = function (gold, i, j, z, x_axis, y_axis, subset = NULL, k = 0.7,simple = "True") 
{
    ...
    if (x_axis == "cor_tmb") {
        ...
        fill_value = "cor_tcell"
        fill_color = "#FB8072"
    }
    else if (x_axis == "cor_pretreat") {
        fill_value = "cor_pretreat"
        fill_color = "#9E7788"
    }
    else if (x_axis == "cor_prolif") {
        fill_value = "cor_prolif"
        fill_color = "#8DD3C7"
    }
    ...
    aesthetic <- aes(x = .data[[x_axis]], y = .data[[y_axis]], 
        color = Main, fill = if_else(.data[[x_axis]] < -1, "cor", 
            "noncor"), size = Type_plus_main, shape = Direction)
    gg <- (ggplot(gold, aesthetic) + geom_point() + scale_color_manual(values = c("black", 
        "white"), guide = "none") + scale_fill_manual(values = c("lightgrey", 
        fill_color), guide = "none") + scale_size_manual(values = unlist(size_map), 
        guide = "none") + scale_shape_manual(values = c(24, 25)) + 
...
ingredients3 = ingredients %>%
    ...
    filter(!feature %in% c("isofox_gene_set_t_cell_effector", "tcell"))
...
fig3a <- (
    super_plot2(
        ingredients3, 
        labels =labels %>% filter(!clean_label %in% c("TGFB", "Proliferation", "Prior Systemic Therapy", "T-cell")), 
        type = "main",
        i = "os", 
        j = "all",
        z = "residuals", 
  ...
  + ggtitle("OS residuals vs All Features")  
