...
Sys.getenv(c("R_LIBS_USER", "R_HOME", "R_PAPERSIZE", "R_PRINTCMD", "HOST"))

?update.packages

update.packages(c("ggh4x"), instlib = "/home/jusset/anaconda3/envs/biomarker_r/lib/R/library")

library(ggh4x)

see <- lsf.str("package:ggh4x")
...
pd.m = pd %>%rownames_to_column("feature") %>% pivot_longer(cols = -feature)
...
pd.m <- pd.m %>% left_join(wdf, by = "feature")
pd.m <- pd.m %>% left_join(wdf[,c(1,2,5)], by = c("name" = "feature"))
...
fig2b <- ggplot(pd.m, aes(x = x, y = y)) +
  ...
  #facet_wrap2(Type ~ Type2, scales = "free", space = "free", switch = "both",render_empty = TRUE,
  facet_wrap2(Type ~ Type2, scales = "free",
